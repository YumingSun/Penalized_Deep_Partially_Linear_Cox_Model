...
scad_exp = function(x_train,y_train,x_test,y_test,select_dim){
  N = dim(x_train)[1]
  p = dim(x_train)[2]
  ...
  LAMBDAS =  exp(seq(log(5), log(0.08), length.out=100))
  #LAMBDAS =  exp(seq(log(0.1),log(0.05), length.out=10))# exp(seq(log(1), log(0.01), length.out=100))
...
# dataLoc = paste0('/nfs/turbo/jiankanggroup/yumsun/',
#                  sprintf("C%d/",signal_strength),
#                  sprintf("p%d/",select_dim))

# resPath = paste0('/home/yumsun/DFS/results/experiment/nonlinear/scad/C90/',
...
if (linear) {
  ...
  resPath = paste0('/home/yumsun/DFS/results/experiment/linear/scad/',
                   sprintf('N%04dP%04d/',train_sample_size,select_dim),
                   'all_res/')
} else{
  dataLoc = '../data'
...
res = scad_exp(x_train,y_train,x_test,y_test,select_dim)
